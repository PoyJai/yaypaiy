...
// 3. ดึงข้อมูลเกมเด่น (Featured Games)
...
<head>
    ...
    <style>
        ...
        .toy-card {
            ...
            border-bottom: 5px solid #41e0fc;
        }
        .toy-card:hover {
...
<body class="text-gray-700">
    ...
    <header class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b-4 border-dashed border-toy-orange shadow-sm">
        <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex space-x-8 text-lg font-semibold items-center">
                ...
                <button id="open-cart-btn" class="relative bg-blue border-2 border-toy-blue p-2 rounded-full hover:bg-toy-blue group transition">
                ...
                <div id="auth-status-container">
                    <?php if ($is_logged_in): ?>
                        <div class="flex items-center space-x-3 bg-toy-pink/50 px-4 py-1 rounded-full border border-toy-orange">
                            <span class="text-sm font-bold">สวัสดี, <?= $current_username ?> 🧸</span>
                            <a href="index.php?logout=1" class="text-xs bg-blue px-2 py-1 rounded-md shadow-sm hover:bg-red-50 text-red-400">ออก</a>
                        </div>
                    <?php else: ?>
                        <a href="login.php" class="px-6 py-2 bg-toy-pink border-b-4 border-pink-400 rounded-2xl font-bold hover:brightness-105 active:border-b-0 transition-all">
                            เข้าสู่ระบบ 🚀
                        </a>
                    <?php endif; ?>
                </div>
    ...
    <main class="container mx-auto px-6 py-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            <?php if (!empty($games)): ?>
                ...
                ?>
                <div class="toy-card bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 p-3">
                    <a href="game_detail.php?id=<?= $game['id'] ?>">
                        ...
                        <div class="px-2 pb-2">
                            ...
                            <div class="flex justify-between items-center mt-3">
                                ...
                                <button class="p-2 bg-toy-yellow rounded-xl hover:bg-yellow-200 transition">
                                    <svg class="w-5 h-5 text-yellow-700" fill="currentColor" viewBox="0 0 20 20"><path d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 100-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 3H6.28l-.31-1.243A1 1 0 005 1H3z"></path></svg>
                                </button>
                            </div>
                        </div>
                    </a>
            ...
            <?php else: ?>
                <div class="col-span-4 text-center py-20 bg-white rounded-3xl border-4 border-dashed border-gray-100">
                    <p class="text-gray-400">ยังไม่มีของเล่นใหม่ๆ เข้ามาเลย รอแป๊บนะ... 🧸</p>
                </div>
            <?php endif; ?>
        </div>
    </main>
    ...
    <div id="cart-modal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-md p-8 rounded-[2rem] shadow-2xl relative border-4 border-toy-blue">
            ...
            <h2 class="text-2xl font-black text-center mb-6 text-gray-700">🛒 ตะกร้าของเล่นของคุณ</h2>
            ...
            <div class="mt-8 pt-6 border-t-2 border-dashed border-gray-100">
                ...
                <button id="checkout-btn" class="w-full py-4 bg-toy-blue text-white font-black text-xl rounded-2xl shadow-[0_4px_0_0_#8ECAE6] hover:translate-y-1 hover:shadow-none transition-all disabled:opacity-50 disabled:grayscale">
                    ไปที่หน้าชำระเงิน ✨
                </button>
            </div>
        </div>
    ...
    const saveCartToStorage = (cart) => localStorage.setItem('game_cart', JSON.stringify(cart));
    ...
    const renderCart = () => {
        ...
        if (cart.length === 0) {
            cartItemsList.innerHTML = '<div class="text-center py-10 text-gray-300">ไม่มีอะไรในตะกร้าเลย 🎈</div>';
            ...
            cart.forEach((item, index) => {
                ...
                cartItemsList.innerHTML += `
                    <div class="flex justify-between items-center bg-toy-yellow/30 p-4 rounded-2xl border border-toy-yellow">
                        <div>
                            <div class="font-bold text-gray-700">${item.title}</div>
                            <div class="text-toy-pink font-black text-sm">฿${price.toFixed(2)}</div>
                        </div>
                        <button onclick="removeItem(${index})" class="text-gray-300 hover:text-red-400 transition">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        renderCart();
        openCartBtn.onclick = () => { renderCart(); cartModal.classList.remove('hidden'); };
        closeCartModalBtn.onclick = () => cartModal.classList.add('hidden');
